...
```
```{r}
sum(is.na(nynoaa$prcp))/nrow(nynoaa)
sum(is.na(nynoaa$snow))/nrow(nynoaa)
sum(is.na(nynoaa$snwd))/nrow(nynoaa)
sum(is.na(nynoaa$tmax))/nrow(nynoaa)
sum(is.na(nynoaa$tmin))/nrow(nynoaa)
```

This dataset has `r nrow(nynoaa)` rows and `r ncol(nynoaa)` columns. It starts from 1981-01-01 to 2010-12-31, containing the weather infomation of these days, including prcp, snow, snwd, tmax, and tmin. In tmax and tmin, the proportion of missing value is up to 43%, which is very high.
